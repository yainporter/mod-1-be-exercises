RSpec.describe 'select pattern' do
  ...
  it 'test 4' do
    ...
    rainbow.each do |position, color|
      if color.length > 4
        greater_than_four.store(position, color)
      end
    end
  ...
  it 'test 6' do
    ...
    furniture.each do |room, furniture|
      if furniture.include?("c")
        words_with_c.store(room, furniture)
      end
    end
  ...
  it 'test 8' do
    ...
    meal.each do |meal, food|
      if food.lines(" ").count == 2
        two_words.store(meal,food)
      end
    end
