...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 7' do
    ...
    # all restaurants
    employee_names = stores.values.map{|restaurant|restaurant[:employees]}.flatten
  ...
  it 'test 8' do
    ...
    ingredients = stores.values.map{|restaurants|restaurants[:dishes]}.flatten.map{|dishes|dishes[:ingredients]}.flatten
    expected = [
      "Rice",
      "Cheese",
      "Butter",
  ...
  it 'test 9' do
    ...
    full_menu_price = stores[:olive_garden][:dishes].map{|dishes| dishes[:price]}.sum
  ...
  it 'test 10' do
    ...
    olive_garden_menu = olive_garden_menu = olive_garden_menu = stores[:olive_garden][:dishes].map{|dish| {dish[:name] => dish}}.pop
    expected = {
      "Risotto" => {
        :name => "Risotto",
